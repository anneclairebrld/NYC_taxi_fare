...
#encode & drop neighbourhood
encode_neighbourhood(train_df)<- function(df){
  neighbour1 <- data.frame(df$X, df$neighborhoods)
  neighbour2 <- dummyVars("~ .", data = neighbour1)
  neighbour3 <- data.frame(predict(neighbour2, newdata = neighbour1))
  names(season3)[names(season3) == "df.X"] <- "X"
  names(season3)[names(season3) == "df.season_feature.summer"] <- "summer"
  names(season3)[names(season3) == "df.season_feature.winter"] <- "winter"
  names(season3)[names(season3) == "df.season_feature.spring"] <- "spring"
...
train_df <- encode_neighbourhood(train_df)
